...
class LibrusClient {
	 ...
	 * @param {Object} [options={}] Constructor options. Cookies or own deviceId.
	 */
	constructor(options = {}) {
		this.bearerToken = "";
		this.pushDevice = -1;
	...
	async cacheCredentials() {
		...
		fs.writeFileSync("./cache.json", obj);
	}
	...
	async loadCachedCredentials() {
		if (fs.existsSync("./cache.json")) {
			const contents = fs.readFileSync("./cache.json");
	 ...
	 * @async
	 * @param {Boolean} [keep=true] Set to false to keep the changes for future getPushChanges calls
	 * @returns {JSON} Array of changes
	 */
	async getPushChanges(keep = true) {
		let result;
		try {
			result = await this.rawRequest(`https://api.librus.pl/2.0/PushChanges?pushDevice=${this.pushDevice}`);
		} catch (error) {
			console.error(error);
			return;
		}
